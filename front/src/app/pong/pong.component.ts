...
export class PongComponent {
  ...
  ngOnInit() {
    ...
    const code = this.route.snapshot.queryParamMap.get('type');
    if (!code) this.socket.emit('joinGame', 'classic');
    else this.socket.emit('privateSocket');
    ...
    this.socket.on('gameUpdate', (data: any) => {
      ...
      if (this.side == 1) this.paddleLeftTop = data.paddleLeftTop / 5;
      else this.paddleRightTop = data.paddleRightTop / 5;
    });
    this.socket.on('gameResult', (data: any) => {
      this.scoreP1 = data.scoreP1;
      this.scoreP2 = data.scoreP2;
      this.gameStatus = 4;
      if (!code) this.socket.emit('endGame', 'classic');
      else this.socket.emit('endGame', 'private');
    });
  }
  ...
  ngOnDestroy() {
    ...
    else if (!code) this.socket.emit('endGame', 'classic');
    else this.socket.emit('endGame', 'private');
  }
  ...
  onKeyDown(event: KeyboardEvent) {
    if (this.gameStatus == 0 || this.gameStatus == 4) return;
    if (
      event.key === 'ArrowDown' &&
      this.paddleLeftTop < 84 &&
      this.side == 0
    ) {
